...
#                 # predictions = np.clip(predictions, 5, None)
#                 # 填充预测值
...
class TaskTimePredictor:
    ...
    def __init__(self, methods, features, model_for_method:Dict[str,str] = {"run_calculator": "svr", "run_sampling":"random_forest","train":"random_forest","evaluate":"random_forest"}):
        """
        初始化任务时间预测器
    ...
    def train(self, train_data: Dict[str, List[Dict]]) -> Dict[str, float]:
        ...
        for method, data in train_data.items():
            ...
            try:
                # GPR需要单独处理y的标准化
                if isinstance(self._models[method].steps[-1][1], GaussianProcessRegressor):
                    # 保存原始统计量到模型对象
                    self._models[method].y_mean_ = y.mean()  # 新增
                    self._models[method].y_std_ = y.std()    # 新增
                    y_scaled = (y - self._models[method].y_mean_) / self._models[method].y_std_
                    self._models[method].fit(X, y_scaled)
                else:
                    self._models[method].fit(X, y)
                    
                scores[method] = self._evaluate_model(method, X, y)
            except Exception as e:
                logger.error(f"Training failed for method {method}: {str(e)}")
                continue
    ...
    def fill_runtime_records_with_predictor(self, method=None):
        ...
        for method in self.methods:
            ...
            try:
                # GPR需要特殊处理
                if isinstance(self._models[method].steps[-1][1], GaussianProcessRegressor):
                    y_pred, y_std = self._models[method].predict(features, return_std=True)
                    # 使用训练阶段保存的统计量
                    adjusted_pred = y_pred * self._models[method].y_std_ + self._models[method].y_mean_
                    predictions = np.expm1(adjusted_pred)
                else:
                    predictions = np.expm1(self._models[method].predict(features))
    ...
    def _evaluate_model(self, method: str, X: np.ndarray, y: np.ndarray) -> float:
        """模型特异性评估"""
        model = self._models[method]
        if isinstance(model.steps[-1][1], GaussianProcessRegressor):
            # GPR返回负对数似然值（越小越好）
